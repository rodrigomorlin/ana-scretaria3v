...
def organizar_escala(req: OrganizarRequest, user=Depends(auth)):
    ...
    estado = {p: {"fim": None, "local": None, "carga": 0, "n": 0, "cirurgiao": ""} for p in profs}
    extras = 0
    aloc = []
    pico, pico_hora = 0, 0
    for c in casos:
        ...
        for p, st in estado.items():
            ...
            score += st["carga"]
            # continuidade de equipe: mesmo cirurgião em sequência → atraso fica contido na própria equipe
            if c["cirurgiao"] and st["cirurgiao"] and st["cirurgiao"].lower() == c["cirurgiao"].lower():
                score -= 300
...
async def create_evento(ev: Evento, bg: BackgroundTasks, request: Request, user=Depends(auth)):
    ...
    if VAPID_PUBLIC_KEY:
        # push individual: o médico designado recebe "para você"; sem vínculo, aviso geral
        alvo_uid = None
        try:
            if ev.doc:
                d = sb_rest("GET", f"/doctors?group_id=eq.{org_id}&active=eq.true&name=eq.{urllib.parse.quote(ev.doc)}&select=user_id")
...
async def run_daily_reminder(org_id: Optional[str] = None):
    ...
    for g in grupos:
        ...
        if not appts:
            continue
        n = len(appts)
        # push individual: cada médico vinculado recebe SÓ os casos dele
        docs_push = sb_rest("GET", f"/doctors?group_id=eq.{gid}&user_id=not.is.null&select=id,name,user_id")
        secs_push = {s["id"]: s["name"] for s in sb_rest("GET", f"/sectors?group_id=eq.{gid}&select=id,name")}
        com_casos = set()
        for d in docs_push:
            ...
            if len(meus) > 3:
                resumo += f" · +{len(meus)-3}"
            await push_user(d["user_id"], f"🩺 Seus casos de amanhã ({len(meus)})", resumo, "/")
        # casos sem médico vinculado: aviso geral ao grupo (alguém precisa assumir)
        orfaos = [a for a in appts if a.get("doctor_id") not in com_casos]
        if len(orfaos) == n:
            await push_all_org(gid, f"🩺 A.N.A · Agenda de {amanha_str}",
                               f"{n} procedimento{'s' if n != 1 else ''} agendado{'s' if n != 1 else ''} para amanhã", "/")
        total_push += 1
        # email por médico (Resend ou SMTP)
        if SMTP_HOST or RESEND_API_KEY:
...
def solicitar_entrada(c: CodigoAcesso, request: Request, bg: BackgroundTasks):
    ...
    if VAPID_PUBLIC_KEY:
        try:
            admins = sb_rest("GET", f"/group_members?group_id=eq.{grupo['id']}&role=eq.admin&select=user_id")
            for a in admins:
                bg.add_task(push_user, a["user_id"], "👤 Solicitação de entrada",
                            f"{u['nome']} pediu para entrar no grupo {grupo['name']} — aprove em Config")
        except Exception:
            pass
